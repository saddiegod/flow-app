...
// ── Utils ─────────────────────────────────────────────────────────────────────
import {
  todayISO, toISO, calDays, recurApplies, uid,
  migrateTasks, recurLabel,
} from "./utils/dates.js";
...
// ── Calendar day button ────────────────────────────────────────────────────────
function CalDay({ d, selected, dot }) {
  const sel = d.iso === selected;
  return (
    <button onClick={()=>sfx("click")} style={{
...
export default function App() {
  ...
  const [focusTask, setFocusTask] = useState(null); // task to open in pomodoro
  const [activeBd,  setActiveBd]  = useState(null); // vision board open in detail

  // ── Data state ──────────────────────────────────────────────────────────────
  // On first load: migrate old locale-format dates to ISO
  ...
  // ── Auto-persist ────────────────────────────────────────────────────────────
  useEffect(()=>{ save(KEY.tasks,  tasks);     }, [tasks]);
  useEffect(()=>{ save(KEY.rec,    recurring); }, [recurring]);
  useEffect(()=>{ save(KEY.habits, habits);    }, [habits]);
  useEffect(()=>{ save(KEY.boards, boards);    }, [boards]);

  // ── Merge once + recurring tasks for selected date ─────────────────────────
  // Performance: only look at tasks within ±45 days of current date view
  const allTasksForDate = useMemo(() => {
    ...
    const rec = recurring
      ...
      .map(rt => ({
        ...
        title:      rt.title,
        date,
        time_start: rt.time_start || null,
        time_end:   rt.time_end   || null,
  ...
  useEffect(() => {
    ...
    notifTimers.current = [];
    allTasksForDate
      .filter(t => t.time_start && t.status !== "done")
      .forEach(t => {
        ...
        if (delay > 0) {
          const id = setTimeout(() =>
            fireNotif(`⏰ En 10 min: ${t.title}`, `Empieza a las ${t.time_start}`, "pf_"+t.id),
            delay
          );
  ...
  // ── Save new task / routine ────────────────────────────────────────────────
  const saveTask = useCallback(({ title, timeStart, timeEnd, category, notes, mode, rType, rDays }) => {
    sfx("click");
    if (mode === "recurring") {
      setRecurring(p => [...p, {
        id:         uid(),
        ...
        notes:      notes     || "",
        // Store rDays as number array — crucial fix
  ...
  const reorderTasks = useCallback(orderedIds => {
    setTasks(prev => {
      const dated = prev.filter(t => t.date === date);
      const other = prev.filter(t => t.date !== date);
      const timed   = dated.filter(t => t.time_start);
      const noTime  = dated.filter(t => !t.time_start);
  ...
  const togglePauseHabit = useCallback(id => {
    setHabits(p => p.map(h => {
      if (h.id !== id) return h;
      if (h.frozen) {
        // Unfreeze: accumulate the frozen duration
        const frozenDuration = Date.now() - (h.frozenAt || Date.now());
        return { ...h, frozen:false, frozenAt:null, pausedMs:(h.pausedMs||0)+frozenDuration };
      } else {
  ...
  const createBoard = useCallback(() => {
    ...
    const nb = {
      id:          uid(),
      title:       "Nueva visión",
      emoji:       BOARD_EMOJIS[boards.length % BOARD_EMOJIS.length],
  ...
  // ── Auth stubs (uncomment + connect Supabase to activate) ─────────────────
  const handleLogin  = async () => { throw new Error("Descomenta el import de Supabase para activar."); };
  const handleLogout = () => setSession(null);

  // ── Active board (always read from latest boards state) ───────────────────
  ...
  return (
    <div style={{ fontFamily:FONT, background:"#080B14", minHeight:"100vh", color:"#F8F9FF", maxWidth:480, margin:"0 auto", position:"relative", overflowX:"hidden" }}>
        ...
        {tab === "today" && (
          <div className="fi">
            ...
            <div style={{ display:"flex",gap:6,marginBottom:20,overflowX:"auto",paddingBottom:4 }}>
              {cal.map(d => (
                <div key={d.iso} onClick={() => setDate(d.iso)}>
                  <CalDay d={d} selected={date} dot={calDots[d.iso]??null}/>
                </div>
              ))}
            </div>
          ...
          <div className="fi">
            ...
            {boards.length === 0 ? (
              ...
              <div style={{ display:"flex",flexDirection:"column",gap:12,marginBottom:28 }}>
                {boards.map(b=>(
                  <BoardCard key={b.id} board={b} onClick={()=>{sfx("click");setActiveBd(b.id);}}/>
                ))}
              </div>
            )}
      ...
      {/* ── Tab Bar ── */}
      <div style={{ position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:480,background:"rgba(8,11,20,.96)",backdropFilter:"blur(28px)",borderTop:"1px solid rgba(255,255,255,.05)",display:"flex",padding:"8px 18px calc(env(safe-area-inset-bottom,0px) + 8px)",zIndex:60 }}>
        {[
          { k:"today", icon:"📅", l:"Hoy" },
          { k:"add",   icon:"✦",  l:"Añadir" },
