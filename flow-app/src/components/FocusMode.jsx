...
// ── Tell the Service Worker to show/update Live Activity ──────────────────────
async function swMessage(data) {
...
// ── Update page title (visible when app is backgrounded) ─────────────────────
function setPageTitle(secs, label) {
  document.title = secs > 0 ? `${fmtTime(secs)} · ${label} — Focus` : "Focus.";
}
...
export default function FocusMode({ task, onClose }) {
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      setSecs(s => {
        ...
        setPageTitle(Math.max(0, next), mode.label);
        if (next <= 0) {
          clearInterval(intervalRef.current);
          setRunning(false);
          sfx("pomodoro");
          swMessage({ type:"POMO_STOP" });
          setPageTitle(0, "");
          if (modeIdx === 0) {
            const ns = sessions + 1;
            setSessions(ns);
            fireNotif("🍅 ¡Pomodoro completado!", `${ns} sesión${ns!==1?"es":""} hoy.`, "pomo");
          } else {
  ...
  useEffect(() => { setSecs(null); setRunning(false); swMessage({type:"POMO_STOP"}); }, [modeIdx, preset]);

  // Restore title on unmount
  useEffect(() => () => { document.title = "Focus."; swMessage({type:"POMO_STOP"}); }, []);

  const toggle = () => {
    sfx("click");
    if (!running) {
      const totalS = current > 0 ? current : durSecs();
      // Start Live Activity in SW
      swMessage({ type:"POMO_START", taskTitle:task?.title||"", totalSecs:totalS, mode:mode.id });
    } else {
      swMessage({ type:"POMO_STOP" });
    }
  ...
  const reset = () => {
    sfx("click"); setRunning(false); setSecs(null);
    swMessage({ type:"POMO_STOP" }); setPageTitle(0,"");
  };
  ...
  return (
    <div style={{
      position:"fixed",inset:0,zIndex:400,background:"#080B14",
      ...
      {sessions > 0 && (
        <div style={{position:"absolute",top:58,left:22,display:"flex",gap:6}}>
          ...
          {sessions > 8 && <span style={{fontSize:10,color:"rgba(255,255,255,.4)",fontWeight:700}}>+{sessions-8}</span>}
        </div>
      )}
      ...
      <div style={{display:"flex",gap:6,marginBottom:52}}>
        {MODES.map((m,i)=>(
          <button key={m.id} onClick={()=>{ setModeIdx(i); }} style={{
            ...
            color:modeIdx===i?"#FFF":"rgba(255,255,255,.35)",transition:"all .25s",
          }}>{m.label}</button>
        ))}
      </div>
      ...
      <div style={{display:"flex",gap:16,marginTop:44,alignItems:"center"}}>
        <button onClick={reset} style={{width:52,height:52,borderRadius:"50%",border:"1px solid rgba(255,255,255,.1)",background:"rgba(255,255,255,.06)",cursor:"pointer",color:"rgba(255,255,255,.45)",fontSize:18,display:"flex",alignItems:"center",justifyContent:"center"}}>↺</button>
        <button onClick={toggle} style={{
          ...
          transition:"transform .15s",transform:running?"scale(.96)":"scale(1)",
        }}>
          {running?"⏸":current===0?"↺":"▶"}
        </button>
        <button onClick={()=>setShowOpts(p=>!p)} style={{width:52,height:52,borderRadius:"50%",border:"1px solid rgba(255,255,255,.1)",background:"rgba(255,255,255,.06)",cursor:"pointer",color:"rgba(255,255,255,.45)",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center"}}>⚙</button>
      </div>
        ...
        <div style={{marginTop:28,display:"flex",gap:8}}>
          {POMODORO_PRESETS.map((p,i)=>(
            <button key={i} onClick={()=>{setPreset(i);setShowOpts(false);}} style={{
              ...
              color:preset===i?"#080B14":"rgba(255,255,255,.5)",transition:"all .2s",
            }}>{p.label}</button>
          ))}
        </div>
      )}
      ...
      {running && (
        <div style={{position:"absolute",bottom:52,fontSize:11,color:"rgba(255,255,255,.25)",fontWeight:600,textAlign:"center"}}>
          ⏰ Recibirás una notificación al terminar<br/>
          <span style={{fontSize:10,color:"rgba(255,255,255,.15)"}}>El timer continúa aunque minimices la app</span>
        </div>
      )}
    </div>
  );
